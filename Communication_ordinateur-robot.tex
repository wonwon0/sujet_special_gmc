...
Mais avant de lancer la machine virtuelle et d'utiliser URsim, il est très important de s'assurer que le contrôleur réseau de la machine vertuelle est en mode "bridged".
Ceci permet à la machine virtuelle de se faire attribuer une adresse IP propre à elle même et rend possible la communication UDP entre Windows et la machine virtuelle.
...
Suivant le dépot du dossier \textit{programs} dans le répertoire d'URsim, il est possible d'ouvrir le programme portant le nom d'\textit{matlabcontrolV3.urp} après avoir sélectionner \textit{Load Program} tel que vu dans la Figure \ref{fig:ui_start}.

Ensuite, il faut executer les deux étapes suivante:
\begin{itemize}
\item Modifier l'adresse IP dans la fonction \textit{socket\_open} du URscript, (visible en Figure \ref{ui_programmation}), pour qu'elle concorde avec l'adresse IP du controleur réseau Windows. Cette infromation est disponible en ouvrant un invité de commande dans la session Windows et de tapper ipconfig.
...
Ces étapes lancent l'URscript sur un robot virtuel et permet l'écoute de commandes envoyées par Matlab.
